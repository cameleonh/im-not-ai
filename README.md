...
>
> **웹에서 쓰고 싶다면?** 2.0부터 **Claude.ai에서도 스킬로 등록**해 쓸 수 있습니다 (서브에이전트 없이 한 스킬이 4단계를 순차 수행하도록 평탄화한 버전). → [Claude.ai에서 쓰기](#claudeai에서-쓰기). 일반 ChatGPT에서는 동작하지 않습니다.
...
Claude.ai의 커스텀 스킬은 서브에이전트·슬래시 커맨드·병렬 팀을 지원하지 않습니다. 그래서 이 리포의 5인 멀티에이전트 파이프라인을 **한 스킬이 4단계(탐지 → 윤문 → 내용 감사 → 자연스러움 리뷰)를 순차로 수행하도록 평탄화**한 자족형 패키지를 따로 제공합니다. 품질 기준(의미 불변·근거 기반·과윤문 방지·등급 A/B/C/D)은 동일합니다.
